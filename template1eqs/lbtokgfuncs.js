...
let lbtokgpreset = {
  ...
  addproblem: addlbtokg,
  ontype: lbtokgtype,
  getanswer: lbtokganswer,
  validate: lbtokgvalidate,
  speechText: lbtokgspeech,
  name: "pounds to kg",
  settings: {
    preset: "easy",
    presets:{
...
function addlbtokg(main=false,self=lbtokgpreset,name=null){
...
function lbtokganswer(problem){
    return problem[0]*0.45359237;
}

function lbtokgvalidate(answer, inputnumber){
  let input = parseFloat(inputnumber);
  let distance = answer - input;

  console.log(distance);

  return distance < lbtokgpreset.settings.range2[1] && distance > lbtokgpreset.settings.range2[0]
}
